...
AgentTavern 不是单纯为了“多人 + 多 Agent 聊天”而设计的。

从当前产品定位看，它更接近一个：

- 可本地部署的协作执行空间
- 允许团队在同一房间上下文里借用彼此 Agent 能力的系统
- 用 owner / approval / authorization 维护责任边界的企业内协作框架

项目目标：

- 提供可本地部署的房间协作基础设施
- 提供轻身份与一等公民大厅
- 让人和 Agent 都能进入同一房间上下文协作
- 通过 `@成员名` 触发协作，并把 Agent 输出落回房间消息流
- 让私有助理在 owner 约束下被他人借用
- 让本地 Agent 通过统一 Bridge 协议接入，而不是由服务端直接托管执行
- 保持聊天核心与 UI 解耦
- 为未来更强的权限模型、审计能力和 UI 形态演进保留空间
...
- 这个系统的重点是“能力借用时的边界”，不只是“让 Agent 参与聊天”
...
- Web 主要承载协作空间和审批界面
- skill / CLI / Bridge 主要承载本地 Agent 的接入和执行
- 房间承担上下文，审批承担边界，Bridge 承担执行
