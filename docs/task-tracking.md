...
如果你不想先通读整份任务清单，可以先看这里。

### 当前状态

- 房间协作、审批链路与本地 Bridge 执行闭环已基本打通
- 主干文档已按当前产品定位完成一轮收口
- 当前更多是在做收尾、回归和边界收紧，而不是重新定义主线

### 当前最重要的事

- 继续收尾轻身份、大厅与房间入口的产品主链路
- 继续做本地 Agent / Bridge 的真实可用性回归
- 继续清理实现边界、文档口径与历史任务表述之间的偏差

### 最近已完成

- `codex_cli`、`claude_code`、`opencode` 接入基线已落地
- Agent SDK 与 Bridge 自动化测试已补强
- README 与开发者文档已完成分层
- 主干文档口径已按当前实现和产品定位同步一轮
...
- 本文档保留了一部分早期任务名称
- 其中出现的 `Codex` / `codex_cli` 有些是历史记录，不一定代表当前系统只支持单一 backend
- 本文档同时承担“当前进行中任务”和“阶段性历史记录”两种用途，阅读时要区分两者
...
项目状态：主链路已经从“房间聊天 MVP”推进到“可本地部署、带审批约束的协作执行闭环”，当前处于收尾与边界收紧阶段。

当前目标：

- 继续保证本地 Agent 接入闭环稳定可用
- 继续保证真实联调和日常使用可持续
- 在不偏离主线的前提下，收紧入口、权限边界与运行时语义
...
- [x] `P1` 前端可用性补强
  ...
  - [x] `@成员` 输入自动补全
  - [x] 成员区显示本地 assistant 运行态

- [-] `P1` 轻身份与大厅主链路实现
  - [x] 落地 identity / presence 基础表结构
  - [x] 为房间成员补 identity 关联
  - [x] 落地首页首次登记邮箱与全局昵称
  - [x] 落地在线一等公民大厅服务端接口基线
