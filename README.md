...
- 本地 `codex_cli` / `claude_code` / `opencode` adapter 基线
- 本地 Bridge 进程骨架
- Codex 助理邀请接入基线
- 多附件上传、图片预览与文件下载
- 面向后续 UI 替换的事件与模型边界
...
当前 Bridge / Agent SDK 已直接支持这些本地后端：
...
- `local_process`：你自己指定命令和参数，适合做最简单的本地进程接入
- `codex_cli` / `claude_code` / `opencode`：面向现成 AI coding agent CLI 的专用适配

目前 README 里直接提到、并且代码已经接通的工具是：
...
这些工具都是可选依赖，不要求每台开发机都全部安装。

当前运行时策略是：

- 某个 backend 的 CLI 缺失时，只会让该任务失败
- Bridge 进程不会因为某一个 CLI 没装而整体崩掉
- 房间里会收到明确失败原因，例如：
...
这意味着你可以只安装自己要开发或验证的那一种工具，而不是一次性准备全套环境。
...
Bridge 本身不内置某一个固定 provider，而是按任务的 `backendType` 选择对应 driver。

当前仓库里已经接好的 driver 包括：
...
- 不需要卸载或禁用整个 Bridge
- 只有分配到该 backend 的任务会失败
- 其他 backend 仍然可以继续正常执行

这对多人协作和混合环境很重要，因为不同开发者本机安装的 AI 工具本来就可能不一样。
...
OpenCode 目前已经支持作为房间 Agent backend 运行，但仓库内的 `skill:install` 还没有单独增加 `--target opencode`。如果你在本机使用 OpenCode，现阶段更适合直接通过已有 session / backend thread 接入，或按 OpenCode 自己的技能目录规则手动放置 skill。
