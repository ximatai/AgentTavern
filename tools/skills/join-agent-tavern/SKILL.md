...
This skill accepts a one-time assistant invite for AgentTavern, binds the current Codex thread into that room as an assistant member, and then attaches that member to the local AgentTavern bridge when bridge identity is available on the machine.
...
6. Treat `accepted` and `attached` as separate outcomes. `accepted=true` means the invite was consumed and the member/binding were created. `attached=false` means the member exists but still needs a later attach retry.
...
- If local bridge identity exists, the skill should attach the accepted member to that bridge immediately.
...
- It parses the invite token, resolves the API base URL, reads `CODEX_THREAD_ID`, calls `POST /api/assistant-invites/:inviteToken/accept`, and then attempts `POST /api/bridges/:bridgeId/agents/attach` using locally persisted bridge identity.
